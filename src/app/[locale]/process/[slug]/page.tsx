...
import { processSteps, getProcessStepBySlug, type ProcessBlock } from "@/data/processSteps";
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      ...
      languages: Object.fromEntries(locales.map((l) => [l, `/${l}/process/${slug}`])),
    },
    openGraph: {
      title,
      description,
...
function BlockRenderer({ block, locale }: { block: ProcessBlock; locale: Locale }) {
  switch (block.kind) {
    case "lead":
    ...
    case "list":
      return (
        <section className={styles.section} data-reveal>
          {block.heading && <h2 className={styles.sectionHeading}>{block.heading[locale]}</h2>}
    ...
    <>
      ...
      <main className={styles.page}>
        <div className="container">
          <Link href={`/${locale}#process`} className={styles.backLink} data-hover>
          ...
          <nav className={styles.allSteps} aria-label={dict.processDetail.allSteps}>
            <div className={styles.allStepsLabel}>{dict.processDetail.allSteps}</div>
                ...
                return (
                  <li key={s.slug}>
                    <Link
                      ...
                      className={[styles.allStepsLink, active ? styles.allStepsLinkActive : ""]
                        .filter(Boolean)
                        .join(" ")}
                    ...
                    >
                      ...
                      <span className={styles.allStepsTitle}>{s.title[locale]}</span>
                    </Link>
                  </li>
                );
              })}
          ...
          {nextStep && (
            <Link href={`/${locale}/process/${nextStep.slug}`} className={styles.next} data-hover>
              <div>
                <div className={styles.nextLabel}>{dict.processDetail.nextStep}</div>
                <div className={styles.nextTitle}>{nextStep.title[locale]}</div>
              </div>
              <svg
                width="32"
                viewBox="0 0 14 10"
